...
public class JwtProvider {
    ...
    public String generateToken(UserPrinciple userPrinciple){
        ...
        return Jwts.builder().setSubject(roleName).claim("userId",userPrinciple.getUser().getId()).setIssuedAt(new Date()).setExpiration(new Date(new Date().getTime()+EXPIRED)).signWith(SignatureAlgorithm.HS256,SECRET_KEY).compact();
    }
